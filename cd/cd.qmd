...
-   자사(위치: 뉴욕)을 방문한 클라이언트들에게 **각자의 공항으로 돌아가는 최적의 항공사 추천**을 위함

-   항공사마다 운항 빈도, 비행 거리, 정시 운항률, 지연 시간과 같은 요소가 달라 종합적인 최적의 항공사 선택이 필요
...
-   **최근 운영 실적이 안정적이고 신뢰할 수 있는 항공사**
    ...
    -   **운영 규모가 큰** 항공사
        -   운영 횟수 & 운영 거리
        -   보유 항공기 수
        -   좌석 수 ![](ditance.png){.absolute bottom="0" right="10" width="450" height="450"} ![](planes.png){.absolute bottom="0" right="520" width="450" height="450"} ![](runtime.png){.absolute bottom="0" right="1030" width="450" height="450"} ![](seat.png){.absolute bottom="11" right="1540" width="450" height="450"}
...
print(carrier_total_count.sort_values)
```
...
carrier_stats = pd.merge(carrier_stats, table_sort, on='carrier')
...
plt.title('주중 항공사별 부문 점수 비교 (최댓값 정규화)')
plt.legend(loc='upper right')

# 각 막대 위에 총점 표시
for i, carrier in enumerate(weekday_sorted['carrier']):
    plt.text(i, weekday_sorted.iloc[i]['total_norm'] + 0.1, 
...
plt.xlabel('항공사')
plt.ylabel('정규화된 값 (0~1)')
plt.title('주말 항공사별 부문 점수 비교 (최댓값 정규화)')
plt.legend(loc='upper right')

# 각 막대 위에 총점 표시
for i, carrier in enumerate(weekend_sorted['carrier']):
    plt.text(i, weekend_sorted.iloc[i]['total_norm'] + 0.1, 
...
plt.title('주중 항공사 부문별 정규화 점수', pad=30) #pad : 위쪽 여백
plt.legend(loc='upper right')
